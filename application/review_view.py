...
class ReasonModal(discord.ui.Modal):
    ...
    def __init__(
        ...
        title = "سبب قبول الطلب" if action == "accept" else "سبب رفض الطلب"
        ...
        self.reason = discord.ui.TextInput(
            label="السبب",
            placeholder="اكتب سبب القرار هنا...",
    ...
    async def on_submit(
        ...
        try:
            ...
            if self.review_view.review_message:
                try:
                    await self.review_view.review_message.edit(
    ...
    async def _complete(
        ...
        if action == "accept":
            ...
            message = (
                "✅ تم قبول الطلب."
                if reason is None
                else "✅ تم قبول الطلب وإرسال السبب للمتقدم."
            )
        else:
            await ResultManager.reject(
                interaction,
            ...
            message = (
                "❌ تم رفض الطلب."
                if reason is None
                else "❌ تم رفض الطلب وإرسال سبب الرفض للمتقدم."
            )
