...
class ScoreView(discord.ui.View):
    def __init__(self, review_view):
        ...
        self.score = 0
        self.max_score = load_config().get("score_max", 10)
    ...
    async def finish(self, interaction, button):
        if not await self.review_view._check_reviewer(interaction): return
        config = load_config()
        minimum = config.get("score_min", 5)
        accepted = self.score >= minimum
        await ResultManager.accept(interaction, self.review_view.user_id, self.review_view.stage, reason=f"⭐ التقييم: {self.score}/{self.max_score}") if accepted else await ResultManager.reject(interaction, self.review_view.user_id, self.review_view.stage, reason=f"⭐ التقييم: {self.score}/{self.max_score} (الحد الأدنى {minimum}/{self.max_score})")
...
class ReasonModal(discord.ui.Modal):
    def __init__(self, action, view):
        ...
        self.action = action; self.review_view = view
    ...
    async def on_submit(self, interaction):
        ...
        self.review_view.disable_all()
        await interaction.response.send_message("✅ تم تنفيذ القرار وإرسال السبب للمتقدم.", ephemeral=True)


class ReviewView(discord.ui.View):
    def __init__(self, user_id, stage):
        super().__init__(timeout=None); self.user_id=user_id; self.stage=stage; self.review_message=None
    ...
    async def scoring(self, interaction, button):
        if not await self._check_reviewer(interaction): return
        config=load_config()
        if not config.get("scoring_enabled",False):
            return await interaction.response.send_message("❌ نظام التقييم غير مفعّل.",ephemeral=True)
