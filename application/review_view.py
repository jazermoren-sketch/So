...
class QuestionScoreView(discord.ui.View):
    def __init__(self, review_view, questions_count):
        ...
        self.questions_count = max(1, int(questions_count))
        self.index = 0
        self.correct = 0
    ...
    async def mark(self, interaction, correct):
        ...
        if self.index >= self.questions_count:
            return await interaction.response.send_message("❌ انتهى تقييم الأسئلة.", ephemeral=True)
        if correct: self.correct += 1
        self.index += 1
        if self.index >= self.questions_count:
            cfg = load_config(); max_score = int(cfg.get("score_max", 10)); minimum = float(cfg.get("score_min", 5))
            score = round((self.correct / self.questions_count) * max_score, 2)
            accepted = score >= minimum
            reason = f"⭐ تقييم الأسئلة: {self.correct}/{self.questions_count} صحيح — {score}/{max_score}"
            if accepted:
                await ResultManager.accept(interaction, self.review_view.user_id, self.review_view.stage, reason=reason, score=score)
            else:
                await ResultManager.reject(interaction, self.review_view.user_id, self.review_view.stage, reason=reason, score=score)
...
class ScoreView(discord.ui.View):
    def __init__(self, review_view):
        ...
        self.review_view = review_view
        self.score = 0
        self.max_score = int(load_config().get("score_max", 10))
    ...
    async def add(self, interaction, button):
        ...
        await interaction.response.edit_message(content=f"⭐ التقييم الحالي: **{self.score}/{self.max_score}**", view=self)
    ...
    async def finish(self, interaction, button):
        if not await self.check(interaction): return
        cfg = load_config(); minimum = float(cfg.get("score_min", 5)); accepted = self.score >= minimum
        reason = f"⭐ التقييم: {self.score}/{self.max_score}" if accepted else f"⭐ التقييم: {self.score}/{self.max_score} (الحد الأدنى {minimum:g}/{self.max_score})"
...
class ReasonModal(discord.ui.Modal):
    def __init__(self, action, view):
        ...
        self.action, self.review_view = action, view
...
class ReviewView(discord.ui.View):
    def __init__(self, user_id, stage):
        ...
        self.user_id, self.stage = user_id, stage
    ...
    async def _check_reviewer(self, interaction):
        cfg = load_config(); reviewers = set(cfg.get("reviewers", []))
        ...
        app = get_application(self.user_id)
    ...
    async def _manual_decision(self, interaction, action, reason=None):
        ...
        if cfg.get("reviewer_voting", False):
            app = get_application(self.user_id) or {}
            votes = app.get("votes", {})
            if str(interaction.user.id) in votes:
                return await interaction.response.send_message("❌ سبق لك التصويت على هذا الطلب.", ephemeral=True)
            votes[str(interaction.user.id)] = action
            app["votes"] = votes; save_application(self.user_id, app)
            approvals = sum(v == "accept" for v in votes.values()); rejections = sum(v == "reject" for v in votes.values())
            required = int(cfg.get("required_approvals", 1))
            if approvals < required and rejections < required:
                return await interaction.response.send_message(f"🗳️ تم تسجيل تصويتك. قبول: **{approvals}/{required}** | رفض: **{rejections}/{required}**", ephemeral=True)
    ...
    async def scoring(self, interaction, button):
        ...
        app = get_application(self.user_id) or {}
        if cfg.get("question_scoring", False):
            return await interaction.response.send_message(f"📝 ابدأ تقييم **{app.get('questions_count', 1)}** سؤالاً.", view=QuestionScoreView(self, app.get('questions_count', 1)), ephemeral=True)
