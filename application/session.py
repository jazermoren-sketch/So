...
class TestSession:
    def __init__(self, interaction, stage, guild_id):
        ...
        self.all_questions=StageManager.get_questions(self.guild_id,stage); self.questions=[]; self.answers=[]; self.index=0
        self.started_at=None; self.timeout_task=None; self.attempt_number=1; self.message_listener=None

    @property
    def session_key(self): return (self.guild_id,self.user.id)

    async def start(self):
        config=load_config(self.guild_id); self.message_listener=getattr(self.interaction.client,"application_message_listener",None)
        existing=get_application(self.guild_id,self.user.id); existing_status=str(existing.get("status","" )).lower() if existing else ""
        if config.get("prevent_duplicate",True) and (self.session_key in active_sessions or existing_status in ("pending","review","قيد المراجعة","testing")):
        ...
        active_sessions[self.session_key]=self; self.started_at=asyncio.get_running_loop().time()
