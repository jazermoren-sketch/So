...
class TestSession:
    ...
    async def start(self):
        ...
        if config.get("prevent_duplicate", True) and (
            self.user.id in active_sessions or
            existing_status in ("pending", "review", "قيد المراجعة", "testing")
        ):
            return await self.interaction.response.send_message(
                "❌ لديك تقديم قيد المراجعة أو اختبار مفتوح بالفعل.", ephemeral=True
            )

        if not self.all_questions:
            return await self.interaction.response.send_message(
                "❌ لا توجد أسئلة لهذه المرحلة.", ephemeral=True
            )

        if existing_status in ("rejected", "مرفوض", "denied"):
            if not config.get("allow_reapply", True):
                return await self.interaction.response.send_message(
                    "❌ إعادة التقديم غير مفعلة حالياً.", ephemeral=True
                )
            ...
            if config.get("test_attempts_enabled", True) and self.attempt_number > max_attempts:
                return await self.interaction.response.send_message(
                    f"❌ استنفدت الحد الأقصى للمحاولات ({max_attempts}).", ephemeral=True
                )

        count = min(int(config.get("test_questions", len(self.all_questions))), len(self.all_questions))
        if config.get("random_questions", True):
            self.questions = random.sample(self.all_questions, count)
        else:
            self.questions = list(self.all_questions[:count])

        active_sessions[self.user.id] = self
        self.started_at = asyncio.get_running_loop().time()
        save_application(self.user.id, {
            "status": "testing", "stage": self.stage, "attempts": self.attempt_number,
            "started_at": self.started_at, "questions_count": len(self.questions),
        })

        await self.interaction.response.send_message(
            f"✅ بدأ الاختبار. عدد الأسئلة: **{len(self.questions)}**.", ephemeral=True
        )

        if config.get("test_timer", True):
            minutes = max(1, int(config.get("test_timer_minutes", 10)))
            self.timeout_task = asyncio.create_task(self._timeout(minutes * 60))
    ...
    async def ask_question(self):
        ...
        embed = discord.Embed(
            title=f"📝 السؤال {self.index + 1}/{len(self.questions)}",
    ...
    async def finish(self, timed_out=False, dm_error=False):
        ...
        status = "failed" if dm_error else "review"
        save_application(self.user.id, {
            "status": status, "stage": self.stage, "attempts": self.attempt_number,
            "questions_count": len(self.questions), "answered": len(self.answers),
            "passed": None, "timed_out": timed_out,
        })
        ...
        text = (
            "❌ تعذر إرسال الرسائل الخاصة. افتح DM وحاول مرة أخرى."
            if dm_error else
            "⏱️ انتهى وقت الاختبار وتم إرسال الإجابات الحالية للمراجعة."
            if timed_out else
            "✅ انتهيت من الاختبار. تم إرسال إجاباتك للمراجعة."
        )
        try:
