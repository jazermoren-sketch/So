...
class ApplicationConfigView(View):
    def __init__(self, guild_id): super().__init__(timeout=300); self.guild_id=str(guild_id); self.config=load_config(self.guild_id)
    def build_embed(self):
        ...
        return embed
    async def toggle(self,interaction,key): self.config[key]=not self.config.get(key,False); save_config(self.guild_id,self.config); await interaction.response.edit_message(embed=self.build_embed(),view=self)
...
class ApplicationCommands:
    ...
    def register(self):
        ...
        async def setreviewchannel(interaction,channel:discord.TextChannel):
            c=load_config(interaction.guild.id); c["review_channel"]=channel.id; save_config(interaction.guild.id,c); await interaction.response.send_message(f"✅ تم تحديد {channel.mention} كروم المراجعة.",ephemeral=True)
        ...
        async def setresultchannel(interaction,channel:discord.TextChannel|None=None):
            c=load_config(interaction.guild.id); c["result_channel"]=channel.id if channel else None; save_config(interaction.guild.id,c); await interaction.response.send_message(f"{'✅ تم تحديد '+channel.mention if channel else '🗑️ تم إلغاء روم النتائج.'}",ephemeral=True)
        ...
        async def setappealchannel(interaction,channel:discord.TextChannel|None=None):
            c=load_config(interaction.guild.id); c["appeal_channel"]=channel.id if channel else None; save_config(interaction.guild.id,c); await interaction.response.send_message(f"{'✅ تم تحديد '+channel.mention+' كروم الاعتراضات.' if channel else '🗑️ تم إلغاء روم الاعتراضات.'}",ephemeral=True)
        ...
        async def setscore(interaction,score_max:app_commands.Range[int,1,100],score_min:app_commands.Range[int,0,100]):
            ...
            c=load_config(interaction.guild.id); c.update({"score_max":score_max,"score_min":score_min}); save_config(interaction.guild.id,c); await interaction.response.send_message(f"✅ **{score_min}/{score_max}** هو الحد الأدنى للقبول.",ephemeral=True)
        ...
        async def setreviewvotes(interaction,approvals:app_commands.Range[int,1,20]):
            c=load_config(interaction.guild.id); c["required_approvals"]=approvals; save_config(interaction.guild.id,c); await interaction.response.send_message(f"✅ عدد الموافقات المطلوبة: **{approvals}**.",ephemeral=True)
        ...
        async def setreapplycooldown(interaction,hours:app_commands.Range[int,0,720]):
            c=load_config(interaction.guild.id); c["reapply_cooldown_hours"]=hours; save_config(interaction.guild.id,c); await interaction.response.send_message(f"✅ مدة الانتظار: **{hours} ساعة**.",ephemeral=True)
        ...
        async def setappealsettings(interaction,cooldown_hours:app_commands.Range[int,1,720],limit:app_commands.Range[int,1,10]):
            c=load_config(interaction.guild.id); c.update({"appeal_cooldown_hours":cooldown_hours,"appeal_limit":limit}); save_config(interaction.guild.id,c); await interaction.response.send_message(f"✅ الاعتراض: كل **{cooldown_hours} ساعة**، والحد **{limit}**.",ephemeral=True)
        @self.bot.tree.command(name="createstage",description="إنشاء مرحلة جديدة")
        @app_commands.checks.has_permissions(administrator=True)
        async def createstage(interaction,name:str): await interaction.response.send_message(f"✅ تم إنشاء المرحلة **{name}**" if StageManager.create_stage(interaction.guild.id,name) else "❌ المرحلة موجودة مسبقاً.",ephemeral=True)
        @self.bot.tree.command(name="deletestage",description="حذف مرحلة")
        @app_commands.checks.has_permissions(administrator=True)
        async def deletestage(interaction,name:str): await interaction.response.send_message(f"🗑️ تم حذف المرحلة **{name}**" if StageManager.delete_stage(interaction.guild.id,name) else "❌ المرحلة غير موجودة.",ephemeral=True)
        @self.bot.tree.command(name="liststages",description="عرض المراحل")
        @app_commands.checks.has_permissions(administrator=True)
        async def liststages(interaction):
            stages=StageManager.list_stages(interaction.guild.id)
        ...
        async def setreviewer(interaction,member:discord.Member):
            c=load_config(interaction.guild.id); c["reviewer"]=member.id; c.setdefault("reviewers",[])
            if member.id not in c["reviewers"]: c["reviewers"].append(member.id)
            save_config(interaction.guild.id,c); await interaction.response.send_message(f"✅ تمت إضافة {member.mention}.",ephemeral=True)
        ...
        async def removereviewer(interaction,member:discord.Member):
            c=load_config(interaction.guild.id); c["reviewers"]=[x for x in c.get("reviewers",[]) if x!=member.id]
            if c.get("reviewer")==member.id: c["reviewer"]=c["reviewers"][0] if c["reviewers"] else None
            save_config(interaction.guild.id,c); await interaction.response.send_message(f"🗑️ تمت إزالة {member.mention}.",ephemeral=True)
        ...
        async def setacceptedrole(interaction,role:discord.Role|None=None):
            c=load_config(interaction.guild.id); c["accepted_role"]=role.id if role else None; save_config(interaction.guild.id,c); await interaction.response.send_message("✅ تم تحديث رتبة القبول." if role else "🗑️ تم إلغاء رتبة القبول.",ephemeral=True)
        ...
        async def setrejectedrole(interaction,role:discord.Role|None=None):
            c=load_config(interaction.guild.id); c["rejected_role"]=role.id if role else None; save_config(interaction.guild.id,c); await interaction.response.send_message("✅ تم تحديث رتبة الرفض." if role else "🗑️ تم إلغاء رتبة الرفض.",ephemeral=True)
        ...
        async def configureapplications(interaction,hidden:bool=False):
            view=ApplicationConfigView(interaction.guild.id); await interaction.response.send_message(embed=view.build_embed(),view=view,ephemeral=hidden)
        ...
        async def settestsettings(interaction,questions:app_commands.Range[int,1,100],pass_percent:app_commands.Range[int,1,100],timer_minutes:app_commands.Range[int,1,180],attempts:app_commands.Range[int,1,20]):
            c=load_config(interaction.guild.id); c.update({"test_questions":questions,"test_pass_percent":pass_percent,"test_timer_minutes":timer_minutes,"test_attempts":attempts}); save_config(interaction.guild.id,c); await interaction.response.send_message("✅ تم حفظ إعدادات الاختبار.",ephemeral=True)
        @self.bot.tree.command(name="applicationstats",description="إحصائيات التقديم")
        @app_commands.checks.has_permissions(administrator=True)
        async def applicationstats(interaction):
            apps=load_applications(interaction.guild.id); total=len(apps); accepted=sum(str(x.get("status")) in ("accepted","مقبول","approved") for x in apps.values()); rejected=sum(str(x.get("status")) in ("rejected","مرفوض","denied") for x in apps.values())
        ...
        for cmd in (setreviewchannel,setresultchannel,setappealchannel,setscore,setreviewvotes,setreapplycooldown,setappealsettings,createstage,deletestage,liststages,setreviewer,removereviewer,setacceptedrole,setrejectedrole,configureapplications,settestsettings,applicationstats): cmd.error(permission_error)
