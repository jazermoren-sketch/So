...
class ResultManager:
    ...
    async def _set_role(interaction, user_id, role_key, remove_role_key=None):
        config=load_config(); member=await ResultManager._get_member(interaction,user_id)
    ...
    async def _send_result_channel(interaction,user_id,status,stage,reason,score):
        config=load_config(); channel_id=config.get("result_channel")
        ...
        embed=discord.Embed(title="📋 نتيجة التقديم",color=discord.Color.green() if status=="accepted" else discord.Color.red())
        embed.add_field(name="👤 المتقدم",value=f"<@{user_id}>",inline=True)
        embed.add_field(name="📌 النتيجة",value=f"{'✅' if status=='accepted' else '❌'} {label}",inline=True)
        embed.add_field(name="📊 التقييم",value=str(score) if score is not None else "غير محدد",inline=True)
    ...
    async def accept(interaction,user_id,stage,reason=None,score=None):
        ...
        await ResultManager._finish_application(user_id,stage,"accepted",reason,interaction.user.id,score)
        await ResultManager._send_result_channel(interaction,user_id,"accepted",stage,reason,score)
    ...
    async def reject(interaction,user_id,stage,reason=None,score=None):
        ...
        if user and load_config().get("application_dm_notifications",True):
            try:
                view=AppealView(user_id) if load_config().get("appeals_enabled",True) and load_config().get("appeal_channel") else None
                await user.send(embed=discord.Embed(title="❌ تم رفض طلبك",description=description,color=discord.Color.red()),view=view)
            except discord.Forbidden: pass
        await ResultManager._finish_application(user_id,stage,"rejected",reason,interaction.user.id,score)
        await ResultManager._send_result_channel(interaction,user_id,"rejected",stage,reason,score)
